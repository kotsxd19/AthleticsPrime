...
const empty = { name: '', contact: '', phone: '', email: '', location: '', active: true };

export default function SupplierModal({ supplier, onClose, onSave }) {
  const [form, setForm] = useState(empty);

  useEffect(() => { setForm(supplier ? { ...supplier } : empty); }, [supplier]);
    ...
    >
      <div className="modal-panel bg-white w-full max-w-lg max-h-[92vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h3 className="text-lg font-bold tracking-tight">{supplier ? 'Editar proveedor' : 'Nuevo proveedor'}</h3>
        ...
        <form onSubmit={e => { e.preventDefault(); onSave({ ...form }); }} className="modal-scroll overflow-y-auto p-6 space-y-5">
          ...
          <div className="form-section">
            <h4><span className="icon-box"><i className="fa-solid fa-truck" /></span>Información del proveedor</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className="input" placeholder="+503 7123 4589"
                  value={form.phone} onChange={e => set('phone', e.target.value)} />
              </div>
              <div>
                <label className="input-label">Correo electrónico</label>
                <input type="email" className="input" placeholder="ventas@empresa.com"
